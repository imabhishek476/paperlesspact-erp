...
export function createComment(
  ...
  return {
    ...
    timeStamp: timeStamp === undefined ? `${formattedTime(new Date())} /${formatDate(new Date())}`: timeStamp,
    type: 'comment',
  };
}
                              ...
                              .map(
                                (
                                  ...
                                  createComment(
                                    ...
                                    innerComment.get('timeStamp') as string,
...
export const getCommentStores = (commentStores : CommentStore[]):Comments =>{
  let temp: Comments = [];
  for(const commentStore of commentStores){
    const t = commentStore.getComments();
    temp = [...temp,...t];
  }
  console.log(temp);
  return temp;
