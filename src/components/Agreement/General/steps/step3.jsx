...
const Step2 = ({
        ...
        Yup.object().shape({
          ...
          phone: Yup.string()
            ...
            .test(
              ...
              function (value) {
                let check = false;
                if (agreementResponse?.party2) {
                  check = agreementResponse?.party2?.some((tenant) => {
  ...
  const [initialValues, setInitialValues] = useState({
    party1Signees: agreementResponse?.party1
      ? agreementResponse?.party1
      : [
          {
            fullname:
  ...
  const onSubmitHandler = async (values, actions) => {
    ...
    const data = {
      ...
      party1: values.party1Signees,
    };
    console.log(values,data);
    const res = await createGeneralAgreement(data, "2");
    if (res) {
            ...
            >
              <FieldArray
                ...
                render={(arrayHelpers) => {
                  return (
                    <div>
                      {formikProps.values.party1Signees.map((signee, index) => (
                        <InputAccordian
                          user={signee}
                          name={"party1Signees"}
                          key={index}
              ...
              <div className="flex justify-start md:justify-end pt-10 pb-3 gap-3">
                <div className="flex gap-3">
                  <Button
                    ...
                    size="md"
                    radius="sm"
                    className="bg-black text-[white] hover:bg-[#E8713C] hover:text-black"
                    onClick={handleBack}
                    disabled={isLoading}
                  >
                    Back
